...
pressure_enclosure_cooldown = ${units 1e-6 Pa} # vacuum
pressure_enclosure_desorption = ${units 1e-3 Pa} # vacuum, assumed
...
# We use a 5 hour cooldown period (same as val-2b) to let the temperature decrease to around 300 K for the start of the desorption.
...
[Functions]
  [temperature_bc_func]
    type = ParsedFunction
    expression = 'if(t<${charge_time},
                            ${temperature_initial},
                  if(t<${fparse charge_time + cooldown_duration},
...
[Postprocessors]
  ...
  [avg_flux_total] # total flux coming out of the sample in atoms/microns^2/s
    ...
    scaling_factor = '${fparse 4 * concentration_scaling / 8}'
  []
  [temperature]
